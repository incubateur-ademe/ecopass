...
test("manage siret delegation", async ({ page }) => {
  ...
  await expect(page.getByTestId("siret-organization-card")).not.toBeVisible()
  await page.getByRole("textbox", { name: "Via son numéro SIRET" }).fill("89964595600025")
  await expect(page.getByTestId("siret-organization-card")).toBeVisible()
  await expect(page.getByTestId("siret-organization-card")).toHaveText(
    "WARO3 RUE JOLIOT-CURIE, 91190 GIF-SUR-YVETTEDéléguer mes droits à cette organisation ",
  )
  await page
    .getByTestId("siret-organization-card")
...
test("manage unique id delegation", async ({ page }) => {
  ...
  await expect(page.getByTestId("uniqueid-organization-card")).not.toBeVisible()
  await page.getByRole("textbox", { name: "Via son identifiant unique" }).fill("350b9fc6-0d05-496b-b429-cc66064e98e8")
  await expect(page.getByTestId("uniqueid-organization-card")).toBeVisible()
  await expect(page.getByTestId("uniqueid-organization-card")).toHaveText(
    "Textile PremiumDéléguer mes droits à cette organisation ",
  )
  await page
    .getByTestId("uniqueid-organization-card")
    .getByRole("button", { name: "Déléguer mes droits à cette organisation" })
    .click()
  await expect(page.getByTestId("to-delegations-table").locator("table tbody tr")).toHaveCount(1)
  await expect(page.getByTestId("to-delegations-table").locator("table tbody tr").locator("td").nth(1)).toHaveText(
    "350b9fc6-0d05-496b-b429-cc66064e98e8",
  )
  await expect(page.getByTestId("from-delegations-table").locator("table tbody tr")).toHaveCount(0)
