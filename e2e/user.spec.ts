...
import { login, loginWithFranceConnectCredentials } from "./utils/login"
...
test("Connection with an other user", async ({ page }) => {
...
test("Connection with a citoyen user", async ({ page }) => {
  ...
  await page.getByRole("link", { name: "Déclaration simplifiée", exact: true }).click()
  await expect(page).toHaveURL(/.*\/declaration-simplifiee/)
  await expect(
    page.getByRole("heading", { name: "Nouvelle déclaration : via formulaire simplifié", exact: true }),
  ).toBeVisible()
