...
import { OrganizationRole, UserType } from "@prisma/enums"
...
test("admin can create a new professionnel user", async ({ page }) => {
  ...
  const user = await prismaClient.user.findUnique({ where: { email: newUserEmail } })
  await expect(user?.type).toEqual(UserType.PROFESSIONNEL)
  await expect(user?.organizationRole).toEqual(OrganizationRole.ADMIN)
})

test("admin can create a new citoyen user", async ({ page }) => {
  await login(page, "ecopass-admin-dev@yopmail.com")
  ...
  await page.getByRole("tab", { name: "Citoyen" }).click()
  await page.getByLabel("Adresse email").nth(1).fill(newUserEmail)

  await page.getByRole("button", { name: "Créer l'utilisateur" }).nth(1).click()
  ...
  const user = await prismaClient.user.findUnique({ where: { email: newUserEmail } })
  await expect(user?.type).toEqual(UserType.CITOYEN)
})
