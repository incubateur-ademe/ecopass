...
test("declare my products", async ({ page }) => {
  ...
  const mailDev = await fetch("http://localhost:1080/email")
  const emails = await mailDev.json()
  expect(emails[0].subject).toContain("Déclaration validée ✅")
  ...
  while (attempts < maxAttempts) {
    ...
    try {
      ...
      console.log(`Essai ${attempts} échoué:`, error)
